...
- Home address
- National ID number
- Date of birth (especially combined with a name or postcode)

If you could use a piece of data to find a specific person, it's PII.

---

## Why does it matter?

Laws like GDPR (UK/EU) say you must protect this data. Storing someone's email address in plain text in a table that 50 analysts can query is a problem. If that table gets breached — or even just accidentally shared — real people's private information is exposed.
...
- You can still see what they were wearing
...
The code (hash) is always 64 characters. It's always the same for the same input. But you cannot work backwards from the hash to get the original email. One-way street.

For analytics, this is fine — you can still count unique customers, group by customer, track behaviour. You just can't see their real email.
...
| No collisions (in practice) | Two different emails won't produce the same hash |
| 256-bit output | Always 64 characters long |

SHA-256 is built into PySpark — no extra libraries needed.
